...
Options:
  ...
  -s <lb>,<ub>
    Computes the solving score of the COPs in <INFO_FILE> by scaling partial
    solutions in the range [lb, ub], where 0 <= lb < lb <= 1.
    By default, lb = 0.25 and ub = 0.75

  -a <lb>,<ub>
    Computes the solving area of the COPs in <INFO_FILE> by scaling partial
    solutions in the range [lb, ub], where 0 <= lb < lb <= 1.
    By default, lb = 0.25 and ub = 0.75
...
def main(args):
    ...
    for o, a in opts:
        if o in ('-h', '--help'):
            ...
            timeout = int(a)
            if timeout <= 0:
                print('Error! Timeout', timeout, 'is non positive.',
                      file=sys.stderr)
                sys.exit(2)
        elif o == '-p':
            path = a
            ...
            ub_feat = float(u)
            if lb_feat >= ub_feat:
                print('Error! Bound', l, 'not greater than', u,
                      file=sys.stderr)
                sys.exit(2)
        elif o == '-s':
            l, u = a.split(',')
            ...
            if not (0 <= lb_score < ub_score <= 1):
                print('Error! The score must be 0 <=', l, '<', u, '<= 1',
                      file=sys.stderr)
                sys.exit(2)
        elif o == '-a':
            l, u = a.split(',')
    ...
    if os.path.exists(kb_path):
        print('Error! Folder', kb_path, 'already exists! Choose ', end=' ',
              file=sys.stderr)
        print('another name or location for the knowledge base!',
              file=sys.stderr)
        sys.exit(2)
