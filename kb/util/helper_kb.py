...
def compute_infos(
    ...
    for row in reader:
        ...
        val = float(row[5])
        values = dict(
            (float(t), v) for (t, v) in list(ast.literal_eval(row[6]).items())
        )

        if check:
            check_invariant(
                'info = unk OR (goal != sat AND info = sat) <==> time = T',
            ...
            check_invariant(
                'goal = sat OR info = unk OR info = uns <==> val = nan',
            ...
            last_val = [
                v for (t, v) in list(values.items()) if t == max(values.keys())
            ]
            check_invariant(
        ...
        if goal != 'sat':
            ...
            kb_cop[inst][solv] = {
                'goal': goal, 'info': info, 'time': time, 'val': val,
                'values': values
            }
            if val == val:
                if val < min_val[inst]:
        ...
        for inst, item in list(kb_cop.items()):
            for solver in list(item.keys()):
                ...
                item[solver]['score'] = get_score(
                    info, val, lb_score, ub_score, min_val[inst],
                    max_val[inst], goal, check
                )
                item[solver]['area'] = get_area(
                    info, lb_area, ub_area, min_values[inst], max_values[inst],
                    goal, time, timeout, values, check
                )
...
def check_invariant(inv, a, b, inst, solv):
    """
    Checks if the property a <==> b holds for instance inst and solver solv.
    """
    if (a and not b) or (b and not a):
        print('Error! Invariant', inv, 'violated by solver', solv,
              'on instance', inst)
...
def get_area(
    ...
    if info in ['uns', 'unb']:
        return time
    scaled_vals = sorted([
        (t, scale(v, lb, ub, min_val, max_val, goal))
