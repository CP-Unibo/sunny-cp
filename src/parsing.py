...
By default, sunny-cp prints the raw FlatZinc solution output produced by the selected solver.

If you want MiniZinc-formatted output, generate/provide the corresponding MiniZinc output model
(.ozn) file with --ozn-file <OZN_FILE>, and pipe sunny-cp through solns2out:

    sunny-cp --ozn-file <OZN_FILE> ... | solns2out <OZN_FILE>

The solns2out helper reads FlatZinc solutions from standard input and translates them using:
...
===============
  ...
  --ozn-file <OZN_FILE>
    Use the specified MiniZinc output model (.ozn) file. By default, sunny-cp
    prints the raw FlatZinc solution output generated by the solver. To obtain
    MiniZinc-formatted output, pipe sunny-cp through solns2out <OZN_FILE>.
